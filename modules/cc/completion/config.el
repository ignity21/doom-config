...
;; Code-completion backends.  `disable-packages!' (in packages.el) turns the
;; unselected package's `use-package!' / `after!' blocks into no-ops, so both
;; configurations can live here without `:if' guards.
