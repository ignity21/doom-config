...
(defun cc/lint-check-registration ()
  (pcase-let* ((`(,reg-top . ,reg-langs) (cc/lint--registered))
               (dir (expand-file-name "config.d" cc/lint-root))
    ...
    (dolist (f fs-top)
      (unless (member f reg-top)
        (cc/lint--add (format "unregistered:config.d/%s" f)
                      "config.d/%s exists but config.el never loads it" f)))
    (dolist (f reg-top)
      (unless (member f fs-top)
        (cc/lint--add (format "dangling-registration:config.d/%s" f)
                      "config.el loads config.d/%s but the file is missing" f)))
    (dolist (l fs-langs)
      (unless (member l reg-langs)
        (cc/lint--add (format "unregistered:config.d/langs/%s.el" l)
                      "config.d/langs/%s.el exists but config.el never loads it" l)))
    (dolist (l reg-langs)
      (unless (member l fs-langs)
        (cc/lint--add (format "dangling-registration:config.d/langs/%s.el" l)
